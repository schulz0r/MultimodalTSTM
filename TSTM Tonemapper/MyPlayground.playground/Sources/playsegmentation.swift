...
func crossoverTBetween(_ g1: Gaussian, _ g2: Gaussian) -> Float? {
    let mu1 = g1.mean
    let mu2 = g2.mean
    let s1 = g1.sigma   // variance
    let s2 = g2.sigma   // variance
...
public func touchingSegmentsLinear(from gmm: [Gaussian]) -> (mins: [Float], maxs: [Float]) {
    ...
    let mins: [Float] = [0.0] + xBoundaries
    let maxs: [Float] = xBoundaries + [1.0]
    return (mins, maxs)
}
